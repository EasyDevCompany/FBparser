...
class Parser():
    ...
    def get_goods_links_from_page(self):

        all_goods_container = self.driver.find_element(
    ...
    def get_goods_data(self, links):
        data = []
        print(f'{len(links)} - ссылок')
        f = open('file.txt', 'w')
        f.close()
        for link in links:
            data.append(self.get_item_data(link))
            f = open('file.txt', 'a')
            f.write(str(data[-1]))
            f.write('\n')
            time.sleep(10)
    ...
    def get_item_data(self, link):
        self.driver.get(link)
        name_price_container = WebDriverWait(self.driver, 10).until(
                lambda driver: driver.find_element(By.XPATH, '//div[@class="xyamay9 x1pi30zi x18d9i69 x1swvt13"]')
            )
        name = name_price_container.find_element(By.XPATH, '//h1/span').text
        images_links = self.get_all_goods_images_links(self.driver)
        ...
        real_estate_info = self.get_real_estate_info(self.driver)
        location = self.get_good_location(self.driver)
        # Getting full description
        if len(self.driver.find_elements(By.XPATH, '//span[text()="Ещё"]')) > 0:
            self.driver.find_element(By.XPATH, '//span[text()="Ещё"]').click()
        if len(self.driver.find_elements(By.XPATH, '//div[@class="x14vqqas"]/preceding-sibling::span')) != 0:
        ...
        saler_link = self.get_saler_link(self.driver)
        return {
            'item_link': link,
            'header': name,
    ...
    def get_all_goods_images_links(self, driver):
        images_links = []
        images_container = driver.find_elements(
            By.XPATH, '//div[@class="x1a0syf3 x1ja2u2z"]/div')
        if len(images_container) == 0:
            WebDriverWait(driver, 10).until(
                    lambda driver: driver.find_element(By.XPATH, '//span[@class="x78zum5 x1vjfegm"]/descendant::img')
                )
    ...
    def get_real_estate_info(self, driver):
        real_estate_info = []
        flag = False
    ...
    def get_good_location(self, driver):
    ...
    def get_saler_link(self, driver):
        driver.find_element(
    ...
    def main(self, query):
        ...
        links = self.scroll_to_the_end_of_page(self.driver, url)
        if not debug:
            app_logger.info("Got links")

        if not debug:
            app_logger.info("Start getting data from links")
        result = self.get_goods_data(links)
        if not debug:
